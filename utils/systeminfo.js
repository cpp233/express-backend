...
const transformCPUTime = async () => {
  ...
  systemInfo.idle = Number((idle / total) * 100).toFixed();
  systemInfo.user = Number((user / total) * 100).toFixed();
  systemInfo.sys = Number((sys / total) * 100).toFixed();
  systemInfo.irq = Number((irq / total) * 100).toFixed();
  if (count <= 0) {
